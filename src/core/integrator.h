...
RGB EstimateDirect(const Scene* scene,const Renderer*renderer,
		...
		const Vector3f& wo,Float rayEpsilon,const BSDF *bsdf,Random& rand,const LightSample& lightSample,
...
RGB UniformSampleAllLights(const Scene *scene,
        ...
        const Normal &n, const Vector3f &wo, Float rayEpsilon,BSDF *bsdf, const Sample *sample, Random &rng,
...
RGB UniformSampleOneLight(const Scene *scene,
        const Renderer *renderer, MemoryArena &arena, const Point &p,
        const Normal &n, const Vector3f &wo, Float rayEpsilon,
