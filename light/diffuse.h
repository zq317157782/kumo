...
private:
	...
	float mArea;
public:
	DiffuseAreaLight(const Transform& l2w,const RGB& le,int ns,const Reference<Shape> shape):AreaLight(l2w,ns){
		mLemit=le;
		mShape=shape;
		mArea=mShape->Area();
	}

	virtual RGB L(const Point &p, const Normal &n, const Vector &w) const override{
